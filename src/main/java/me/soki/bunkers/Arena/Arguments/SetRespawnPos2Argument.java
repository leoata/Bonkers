...
public class SetRespawnPos2Argument {
    public static void SetRespawnPos2Argument(CommandSender sender, Command command, String label, String[] args) {
        Player p = (Player) sender;
        if (args.length == 2){
            Team targetTeam = Team.getTeam(args[1]);
            if (targetTeam != null){
                targetTeam.setTeamRespawnPos2(p.getLocation());
                p.sendMessage(ChatColor.YELLOW + "Set " + targetTeam.getColor() + targetTeam.getTeamName() + ChatColor.YELLOW + "'s second respawn position.");
                ...
                getPlugin().getConfig().set(targetTeam.getTeamName() +  ".respawnpos2", xyz);
            }else{
